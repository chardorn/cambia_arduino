...
int sendPacket(uint8_t *buf, uint8_t length) 
{
  ...
  DeserializationError error = deserializeJson(jsonBuffer, bufPtr);
	...
	const char * data	= jsonBuffer["txpk"]["data"];			// Downstream Payload
	uint8_t psize		= jsonBuffer["txpk"]["size"];
	bool ipol			= jsonBuffer["txpk"]["ipol"];
	uint8_t powe		= jsonBuffer["txpk"]["powe"];			// e.g. 14 or 27
	LoraDown.tmst		= (uint32_t) jsonBuffer["txpk"]["tmst"].as<unsigned long>();
	const float ff		= jsonBuffer["txpk"]["freq"];			// eg 869.525
	
	// Not used in the protocol of Gateway TTN:
	const char * datr	= jsonBuffer["txpk"]["datr"];			// eg "SF7BW125"
	const char * modu	= jsonBuffer["txpk"]["modu"];			// =="LORA"
	const char * codr	= jsonBuffer["txpk"]["codr"];			// e.g. "4/5"
